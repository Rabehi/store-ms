package rabehi.store.shoppingms.controller;
...
import rabehi.store.shoppingms.service.InvoiceService;
...
public class InvoiceRest {
    ...
    public ResponseEntity<Invoice> createInvoice(@Valid @RequestBody Invoice invoice, BindingResult result) {
        ...
        if (result.hasErrors()){
            throw new ResponseStatusException(HttpStatus.SC_BAD_REQUEST, this.formatMessage(result), null);
        }
        Invoice invoiceDB = invoiceService.createInvoice (invoice);

        return  ResponseEntity.status( HttpStatus.SC_CREATED).body(invoiceDB);
    }
